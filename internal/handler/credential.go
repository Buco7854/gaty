...
func (h *CredentialHandler) GetMyEffectiveAuthConfig(ctx context.Context, input *workspaceSelfCredPathParam) (*myEffectiveAuthConfigOutput, error) {
	...
	if err != nil {
		return nil, huma.Error500InternalServerError("failed to load membership", err)
	}
...
func (h *CredentialHandler) validateMembership(ctx context.Context, membershipID, workspaceID uuid.UUID) error {
	...
	if err != nil {
		return huma.Error500InternalServerError("failed to validate membership", err)
	}
	return nil
}
...
func (h *CredentialHandler) checkAPITokenEnabled(ctx context.Context, membershipID, workspaceID uuid.UUID) error {
	...
	if err != nil {
		return huma.Error500InternalServerError("failed to load membership", err)
	}
