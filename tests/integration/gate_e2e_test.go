...
//
//  1. Create an admin member via setup/init (or login if already exists)
//  2. Create a gate with HTTP_INBOUND status push
//  3. Subscribe to SSE and watch events in real-time  (visible in -v output)
//  4. Push gate status updates and verify they arrive via SSE
//  5. Verify status rules override reported status
//  6. Verify token rotation invalidates old token
//  7. Cleanup (delete gate)
//
// Run with:
//
...
// SSE routes:
//
//	POST /api/events/ticket  → {"ticket":"..."}
//	GET  /api/events?ticket=...
func subscribeSSE(ctx context.Context, t *testing.T, c *testClient) <-chan SSEEvent {
	t.Helper()

	ticketResp := c.do(http.MethodPost, "/api/events/ticket", nil)
	...
	go func() {
		...
		url := fmt.Sprintf("%s/api/events?ticket=%s", c.base, ticket)
...
// bootstrap creates an admin member via setup/init (or logs in if already set up)
// and returns the authenticated client.
func bootstrap(t *testing.T) *testClient {
	t.Helper()
	c := newTestClient(t)

	ts := fmt.Sprintf("%d", time.Now().UnixNano())
	username := fmt.Sprintf("e2e-%s", ts[:12])
	...
	if setupStatus["setup_required"] == true {
		t.Logf("[bootstrap] first-run setup with %s", username)
		c.call(http.MethodPost, "/api/setup/init", map[string]any{
			"username": username, "password": password,
		}, http.StatusOK)
	} else {
		c.call(http.MethodPost, "/api/auth/login", map[string]any{
			"username": username, "password": password,
		}, http.StatusOK)
		t.Logf("[bootstrap] logged in as %s", username)
	}
...
func TestGateLifecycleLive(t *testing.T) {
	...
	gateResp := c.call(http.MethodPost, "/api/gates", map[string]any{
		"name":             "e2e-live-gate",
	...
	t.Cleanup(func() {
		...
		c.call(http.MethodDelete, fmt.Sprintf("/api/gates/%s", gateID), nil, http.StatusNoContent)
	})

	// ── 2. Initial status ─────────────────────────────────────────────
	section(t, "2 · initial status = unknown (never seen)")
	getResp := c.call(http.MethodGet, fmt.Sprintf("/api/gates/%s", gateID), nil, http.StatusOK)
	...
	finalResp := c.call(http.MethodGet, fmt.Sprintf("/api/gates/%s", gateID), nil, http.StatusOK)
	...
	rotateResp := c.call(http.MethodPost, fmt.Sprintf("/api/gates/%s/token/rotate", gateID), nil, http.StatusOK)
...
func TestGateCreateGetDeleteCycle(t *testing.T) {
	...
	// PATCH rename
	updated := c.call(http.MethodPatch, fmt.Sprintf("/api/gates/%s", gateID),
	...
	resp := c.do(http.MethodGet, fmt.Sprintf("/api/gates/%s", gateID), nil)
	resp.Body.Close()
...
func TestStatusRulesOverride(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("battery=%.0f", tc.battery), func(t *testing.T) {
			...
			resp := c.call(http.MethodGet, fmt.Sprintf("/api/gates/%s", gateID), nil, http.StatusOK)
